...
class TestLabelingWorkflow:
    ...
    def test_bug_without_reproduction_steps(self):
        labels = [
            Label(
                name="bug",
                description="Something isn't working.",
    ...
    def test_never_apply_label(self):
        labels = [
            Label(
                name="do-not-merge",
    ...
    def test_multiple_valid_labels(self):
        ...
        pr = PullRequest(
            ...
            files={
                ...
                "docs/dependencies.md": "updated docs",
            },
            author="marvin",
        )
    ...
    def test_security_label_with_instructions(self):
        labels = [
            Label(
                name="security",
    ...
    def test_complex_feature_pr(self):
        labels = [
            Label(name="enhancement", description="New feature or request"),
            Label(
        ...
        pr = PullRequest(
            ...
            files={
                ...
                "tests/test_database.py": """
                    ...
                    async def test_async_batch_query():
                        client = AsyncDatabaseClient()
                        batch_result = await client.batch_query()
    ...
    def test_codeowners_based_labels(self):
        labels = [
            Label(
    ...
    def test_security_review_routing(self):
        labels = [
            Label(
    ...
    def test_directory_based_labels(self):
        labels = [
            Label(
                name="ci",
    ...
    def test_monorepo_package_labels(self):
        labels = [
            Label(
                name="pkg/core",
    ...
    def test_irrelevant_labels(self):
        labels = [
            Label(
                name="ci",
                description="CI/CD changes",
            ...
            Label(
                ...
                instructions="Apply when there are changes to documentation files",
            ),
            Label(
                name="dependencies",
                description="Dependency updates",
                instructions="Apply when there are changes to dependency files",
            ),
            Label(
                name="performance",
                description="Performance improvements",
            ...
            Label(
                name="bug",
                description="Something isn't working",
                instructions="Apply when there are changes that fix bugs",
            ),
            Label(name="1", description="Irrelevant label"),
            Label(name="2", description="Irrelevant label"),
            Label(name="3", description="Irrelevant label"),
            Label(name="4", description="Irrelevant label"),
        ]
    ...
    def test_insufficient_test_coverage(self):
        labels = [
            Label(
                name="needs-tests",
                description="More tests needed",
                instructions="Apply this label when new functions are added without corresponding test coverage",
            ),
            Label(name="enhancement", description="New feature or request"),
        ]

        pr = PullRequest(
            title="Add new database utilities",
            body="Added several utility functions for database operations",
            files={
                "src/database.py": """
                ...
                "tests/test_database.py": """
                    ...
                    # Missing tests for execute_query and close_connections
                """,
            },
            author="marvin",
        )
        ...
        result = labeling_workflow(item=pr, labels=labels)
        assert "needs-tests" in result
        assert "enhancement" in result

    def test_insufficient_test_coverage_with_custom_instructions(self):
        labels = [
            Label(
                name="needs-tests",
                description="More tests needed",
        ...
        # Custom instructions that override when to apply needs-tests
        custom_instructions = """
        When evaluating test coverage:
        - Only apply 'needs-tests' if there are NO tests at all for new code
        """

        result = labeling_workflow(
            item=pr, labels=labels, instructions=custom_instructions
        )

        # Since there is at least one test (for connect_db),
        # needs-tests should not be applied
        assert "needs-tests" not in result
        assert "enhancement" in result
